...
function StatusBadge({ status }: { status: MemberStatus }) {
  const map: Record<MemberStatus, { label: string; cls: string; Icon: typeof Clock }> = {
    ...
    ACCEPTED: { label: 'Aceptó',    cls: 'bg-emerald-500/20 text-emerald-400', Icon: CheckCircle2 },
...
function PaidBadge({ hasPaid }: { hasPaid: boolean }) {
  return hasPaid ? (
    ...
    <span className="inline-flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full bg-slate-600/40 text-slate-400">
    ...
    <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden">
      <div className="p-5 flex flex-col md:flex-row md:items-center gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="text-right">
            ...
            <p className="text-lg font-extrabold text-emerald-400">{fmt(shared.costPerMember)}</p>
          </div>
          <button
      ...
      {expanded && (
        <div className="border-t border-slate-700 p-5 space-y-3">
          <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-3">Miembros invitados</p>
          {shared.members.length === 0 ? (
            <p className="text-slate-500 text-sm">Sin miembros aún.</p>
          ) : (
            shared.members.map((m) => (
              <div key={m.id} className="flex items-center justify-between gap-3 bg-slate-900/50 rounded-xl px-4 py-3">
                <div>
                  <p className="text-sm font-medium text-white">{m.user?.name ?? m.email}</p>
                  {m.user?.name && <p className="text-xs text-slate-500">{m.email}</p>}
                </div>
                <div className="flex items-center gap-2 flex-wrap justify-end">
                  ...
                  {m.status === 'ACCEPTED' && (
                    <button
                      ...
                      className="text-xs px-2.5 py-1 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-300 transition-all font-medium"
                    >
...
function InvitedCard({ inv, onRespond, onTogglePaid }: {
  ...
  return (
    <div className="bg-slate-800 rounded-2xl border border-slate-700 p-5 flex flex-col md:flex-row md:items-center gap-4">
      <div className="flex-1">
        <div className="flex items-center gap-3 mb-1">
        ...
        <div className="flex flex-wrap gap-3 text-sm text-slate-400">
          <span>Compartida por: <strong className="text-slate-300">{sub.owner.name ?? sub.owner.email}</strong></span>
          <span>·</span>
      ...
      <div className="flex items-center gap-3 shrink-0 flex-wrap justify-end">
        <div className="text-right">
          <p className="text-xs text-slate-500">Tu parte</p>
          <p className="text-lg font-extrabold text-emerald-400">{fmt(inv.costPerMember)}</p>
        </div>
        ...
        {inv.status === 'PENDING' && (
          <>
            <button
              ...
              className="bg-emerald-500 hover:bg-emerald-400 text-white text-xs font-bold px-3 py-2 rounded-lg transition-all"
            >
              Aceptar
            </button>
            <button
              onClick={() => onRespond(inv.id, 'DECLINED')}
              className="bg-slate-700 hover:bg-slate-600 text-slate-300 text-xs font-bold px-3 py-2 rounded-lg transition-all"
            >
              Rechazar
            </button>
          </>
        ...
        {inv.status === 'ACCEPTED' && (
          <>
            ...
            <button
              ...
              className="text-xs px-2.5 py-1.5 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-300 transition-all font-medium"
            >
...
export default function SharedSubscriptions() {
  ...
  const tabCls = (active: boolean) =>
    `px-5 py-2.5 rounded-xl font-bold text-sm transition-all ${
      active ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-slate-800 text-slate-400 hover:text-white'
    }`;
    ...
    <div className="p-8 text-white font-sans max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row items-start md:items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          ...
          <p className="text-slate-400 mt-1 text-sm">Gestiona gastos recurrentes divididos con tu familia o amigos</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        mine.length === 0 ? (
          <div className="text-center py-20 text-slate-500 bg-slate-800/50 rounded-2xl border border-slate-700">
            ...
            <p className="text-lg font-semibold text-slate-400">Aún no compartes ninguna suscripción</p>
            <p className="text-sm mt-1">Ve a Suscripciones y usa el botón <strong className="text-emerald-400">Compartir</strong> en cualquier servicio.</p>
          </div>
        ) : (
        ...
        invited.length === 0 ? (
          <div className="text-center py-20 text-slate-500 bg-slate-800/50 rounded-2xl border border-slate-700">
            <Users size={48} className="mx-auto mb-4 opacity-20" />
            <p className="text-lg font-semibold text-slate-400">No tienes invitaciones de suscripciones</p>
