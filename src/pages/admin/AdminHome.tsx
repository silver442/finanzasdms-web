...
const LEVEL_GROUPS = [
  ...
  { label: 'Cumplidores', keys: ['CUMPLIDOR_1', 'CUMPLIDOR_2', 'CUMPLIDOR_3'], color: 'bg-brand-green', text: 'text-brand-green-light', bar: 'bg-brand-green' },
  { label: 'Socios',      keys: ['SOCIO_1', 'SOCIO_2'],                        color: 'bg-brand-violet-400',   text: 'text-brand-violet',   bar: 'bg-brand-violet-400'   },
...
export default function AdminHome() {
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center py-24 text-text-secondary">
        Cargando métricas...
      </div>
    );
  }
  ...
  return (
    <div className="p-8 text-white font-sans max-w-6xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-text-secondary mt-2">Salud financiera del negocio en tiempo real</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        <div className="bg-surface-card border border-surface-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs text-text-secondary uppercase tracking-wider font-semibold">Capital Prestado</p>
          </div>
          <p className="text-2xl font-extrabold text-white">{fmt(metrics.capitalEnCalle)}</p>
          <p className="text-xs text-text-muted mt-1">saldo pendiente en préstamos activos</p>
        </div>

        <div className="bg-surface-card border border-surface-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <TrendingUp size={16} className="text-brand-green-light" />
            <p className="text-xs text-text-secondary uppercase tracking-wider font-semibold">Ganancia Proyectada</p>
          </div>
          <p className="text-2xl font-extrabold text-brand-green-light">{fmt(metrics.gananciaProyectada)}</p>
          <p className="text-xs text-text-muted mt-1">intereses pendientes por cobrar</p>
        </div>

        <div className="bg-surface-card border border-surface-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <CheckCircle2 size={16} className="text-sky-400" />
            <p className="text-xs text-text-secondary uppercase tracking-wider font-semibold">Recuperación</p>
          </div>
          <p className={`text-2xl font-extrabold ${metrics.recoveryPct >= 80 ? 'text-brand-green-light' : metrics.recoveryPct >= 60 ? 'text-brand-violet' : 'text-red-400'}`}>
            {metrics.recoveryPct}%
          </p>
          <p className="text-xs text-text-muted mt-1">cuotas pagadas vs. totales</p>
        </div>

        <div className="bg-surface-card border border-surface-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <Activity size={16} className="text-brand-violet" />
            <p className="text-xs text-text-secondary uppercase tracking-wider font-semibold">Préstamos Activos</p>
          </div>
          <p className="text-2xl font-extrabold text-white">{metrics.activeLoansCount}</p>
          <p className="text-xs text-text-muted mt-1">de 20 cupos disponibles</p>
        </div>

        <div className="bg-surface-card border border-surface-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <ShieldCheck size={16} className="text-brand-violet" />
            <p className="text-xs text-text-secondary uppercase tracking-wider font-semibold">Administradores</p>
          </div>
          <p className="text-2xl font-extrabold text-white">{adminCount ?? '—'}</p>
          <p className="text-xs text-text-muted mt-1">usuarios con rol admin</p>
        </div>
      </div>
        ...
        <div className="bg-surface-card border border-surface-border rounded-2xl p-6">
          ...
          {totalUsers === 0 ? (
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2 border-t border-surface-border flex justify-between text-xs text-text-muted">
      ...
      {metrics.usersByLevel.length > 0 && (
        <div className="bg-surface-card border border-surface-border rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-surface-border">
            <h3 className="text-sm font-bold text-text-primary uppercase tracking-wider">Desglose Completo por Nivel</h3>
          </div>
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-px bg-surface-elevated">
            {metrics.usersByLevel.map((entry) => {
              const group = LEVEL_GROUPS.find((g) => g.keys.includes(entry.level));
              return (
                <div key={entry.level} className="bg-surface-card p-4 text-center">
                  <p className={`text-xs font-semibold uppercase tracking-wider mb-1 ${group?.text ?? 'text-text-secondary'}`}>
