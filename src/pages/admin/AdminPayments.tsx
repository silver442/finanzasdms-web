...
export default function AdminPayments() {
  ...
  return (
    <div className="p-8 text-white font-sans max-w-5xl mx-auto">
      ...
      {isLoading ? (
        <div className="flex justify-center py-20 text-text-secondary">Cargando abonos...</div>
      ) : requests.length === 0 ? (
        ...
        <div className="space-y-4">
          {requests.map(req => {
            ...
            return (
              <div key={req.id} className="bg-surface-card border border-surface-border rounded-2xl p-5 flex flex-col sm:flex-row gap-5 items-start sm:items-center">
                ...
                <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <p className="text-xs text-text-muted uppercase font-semibold">Cliente</p>
                      <p className="text-white font-semibold text-sm">{userName}</p>
                      {req.user.phone && <p className="text-text-secondary text-xs">{req.user.phone}</p>}
                    </div>
                  </div>

                  {/* Préstamo */}
                  <div>
                    <p className="text-xs text-text-muted uppercase font-semibold mb-0.5">Préstamo · Cuota</p>
                    <p className="text-white text-sm font-medium">{req.installment.loan.concept}</p>
                    <p className="text-text-secondary text-xs">Cuota #{req.installment.number}</p>
                  </div>

                  {/* Monto y banco */}
                  <div>
                    <p className="text-xs text-text-muted uppercase font-semibold mb-0.5">Abono</p>
                    <p className="text-brand-green-light font-bold text-lg">{fmt(req.amount)}</p>
                    {req.adminBank && (
                    ...
                    {req.reference && (
                      <p className="text-xs text-brand-violet font-mono mt-0.5">Ref: {req.reference}</p>
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    {req.user.id !== currentAdminId ? (
                      <button
                        ...
                        disabled={isProcessing}
                        className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-brand-green hover:bg-brand-green-light text-white text-sm font-bold transition-all shadow-lg shadow-brand-green/20 disabled:opacity-40"
                      >
                        <CheckCircle2 size={15} />
                        {isProcessing ? 'Procesando...' : 'Aprobar'}
                      </button>
                    ) : (
                      <span className="px-3 py-2 rounded-xl bg-surface-elevated/50 border border-surface-border text-text-secondary text-xs font-semibold">
                        Tu préstamo
                      </span>
                    )}
                  </div>
      ...
      {receiptPreview && (
        ...
        >
          <div
            className="bg-surface-card border border-surface-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col"
