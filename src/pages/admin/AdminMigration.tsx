...
export default function AdminMigration() {
  ...
  const inputCls = 'w-full bg-surface-base border border-surface-border text-white rounded-lg px-4 py-2.5 focus:outline-none focus:border-violet-500 transition-colors placeholder:text-text-muted';
      ...
      <form onSubmit={e => void handleSubmit(e)} className="space-y-6">
        ...
        <div className="bg-surface-card border border-surface-border rounded-2xl p-6">
          ...
          <div>
            ...
            {loadingUsers ? (
              <div className="flex items-center gap-2 text-text-muted text-sm py-2.5">
          ...
          {selectedUser && (
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className="bg-surface-base rounded-lg p-3 text-sm">
                <p className="text-text-muted text-xs mb-0.5">Límite de Crédito</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input type="date" value={form.startDate} onChange={set('startDate')}
                className={inputCls} required />
              <p className="text-xs text-text-muted mt-1">Puede ser una fecha en el pasado</p>
            </div>

            <div>
              <label className={labelCls}>
                Cuotas ya pagadas
                <span className="text-text-muted normal-case font-normal ml-1">(opcional)</span>
              </label>
        ...
        {hasTable && (
          <div className="bg-violet-500/10 border border-violet-500/30 rounded-2xl p-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
              <div className="bg-surface-base/60 rounded-xl p-4 text-center">
                <p className="text-text-muted text-xs mb-1 uppercase tracking-wider">Total a Pagar</p>
                <p className="text-violet-300 font-bold text-lg">{fmt(totalDebt)}</p>
                <p className="text-text-muted text-xs mt-0.5">Capital: {fmt(amount)} · Interés: {fmt(totalDebt - amount)}</p>
              </div>
              <div className="bg-surface-base/60 rounded-xl p-4 text-center">
                <p className="text-text-muted text-xs mb-1 uppercase tracking-wider">Ya Liquidado</p>
                <p className="text-brand-green-light font-bold text-lg">{fmt(paidAmount)}</p>
                <p className="text-text-muted text-xs mt-0.5">{paidCount} cuota{paidCount !== 1 ? 's' : ''} pagada{paidCount !== 1 ? 's' : ''}</p>
              </div>
              <div className="bg-surface-base/60 rounded-xl p-4 text-center">
                <p className="text-text-muted text-xs mb-1 uppercase tracking-wider">Saldo Pendiente</p>
                <p className={`font-bold text-lg ${pending > 0 ? 'text-brand-violet' : 'text-brand-green-light'}`}>{fmt(pending)}</p>
                <p className="text-text-muted text-xs mt-0.5">{periods - paidCount} cuota{(periods - paidCount) !== 1 ? 's' : ''} por cobrar</p>
              </div>
            </div>
            <p className="text-xs text-text-muted text-center">
              ...
              {paidCount > 0 && (
                <>, con <span className="text-brand-green-light font-semibold">{paidCount} cuota{paidCount !== 1 ? 's' : ''} ya liquidada{paidCount !== 1 ? 's' : ''}</span> y un saldo pendiente de <span className="text-brand-violet font-semibold">{fmt(pending)}</span></>
              )}
            </p>
          </div>
        )}
        ...
        {projection.length > 0 && (
          <div className="bg-surface-card border border-surface-border rounded-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-surface-border flex items-center justify-between">
              <h2 className="text-sm font-bold text-text-primary">Tabla de Amortización</h2>
              <div className="flex items-center gap-4 text-xs text-text-muted">
                <span className="flex items-center gap-1.5">
                  <CheckCircle2 size={12} className="text-brand-green-light" /> Pagada
                </span>
                <span className="flex items-center gap-1.5">
                  <Clock size={12} className="text-text-secondary" /> Pendiente
                </span>
              </div>
            </div>
                ...
                <tbody>
                  {projection.map(inst => (
                    ...
                    >
                      ...
                      <td className={`px-6 py-3 ${inst.paid ? 'line-through text-text-muted' : 'text-text-200'}`}>
                        {fmtDate(inst.dueDate)}
                      </td>
                      <td className={`px-6 py-3 text-right font-semibold tabular-nums ${inst.paid ? 'text-text-muted' : 'text-white'}`}>
                        {fmt(inst.amountDue)}
                      </td>
                      <td className="px-6 py-3 text-center">
                        {inst.paid ? (
                          <span className="inline-flex items-center gap-1 bg-brand-green/15 text-brand-green-light text-xs px-2 py-0.5 rounded-full border border-brand-green/20 font-semibold">
                            <CheckCircle2 size={10} /> Pagada
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 bg-surface-elevated/50 text-text-secondary text-xs px-2 py-0.5 rounded-full border border-surface-border/40 font-semibold">
            ...
            {/* Totales */}
            <div className="px-6 py-4 border-t border-surface-border bg-surface-base/40 grid grid-cols-3 gap-4 text-sm">
              <div>
                <p className="text-text-muted text-xs mb-0.5">Total cuotas</p>
                <p className="text-white font-bold">{periods}</p>
              </div>
              <div>
                <p className="text-text-muted text-xs mb-0.5">Cuota {freqLabel}</p>
                <p className="text-white font-bold">{fmt(periodPay)}</p>
              </div>
              <div className="text-right">
                <p className="text-text-muted text-xs mb-0.5">Total a pagar</p>
