...
function progressColor(ratio: number) {
  ...
  if (ratio >= 0.5) return { bar: 'bg-amber-500', text: 'text-amber-400' };
  ...
  return (
    <div className="mb-4">
      ...
      <div className="flex justify-between mt-1">
        <span className="text-xs text-slate-500">{MXN(current)}</span>
        <span className="text-xs text-slate-500">Meta: {MXN(min)}</span>
      </div>
    </div>
  );
}
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">Saldo promedio y gasto mínimo por periodo de corte</p>
        </div>
      </div>

      {cashItems.length === 0 && (
        <div className="text-center py-16 text-slate-500">No tienes cuentas corrientes registradas aún.</div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex justify-between items-center mb-5">
              ...
              <button onClick={() => setRulesTarget(null)} className="text-slate-400 hover:text-white">
                <X size={20} />
              </button>
            </div>
            <p className="text-slate-400 text-sm mb-4">{rulesTarget.name}</p>

            <label className="block text-xs text-slate-400 mb-1">Día de corte (1–28)</label>
            <input
              type="number"
              min={1}
              max={28}
              ...
              className="w-full bg-slate-900 border border-slate-600 focus:border-emerald-500 rounded-lg px-4 py-2 text-white mb-4 outline-none"
            />

            <label className="block text-xs text-slate-400 mb-1">Saldo promedio mínimo ($)</label>
            <input
              type="number"
              min={0}
            ...
            <label className="block text-xs text-slate-400 mb-1">Gasto mínimo mensual ($)</label>
            <input
              type="number"
              min={0}
            ...
            <button
              ...
              className="w-full bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2.5 rounded-xl font-bold transition-all disabled:opacity-50"
            >
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex justify-between items-center mb-5">
              ...
              <button onClick={() => setMovTarget(null)} className="text-slate-400 hover:text-white">
                <X size={20} />
              </button>
            </div>
            <p className="text-slate-400 text-sm mb-4">{movTarget.name}</p>
    ...
    <div className="bg-slate-800 border border-slate-700 rounded-2xl p-5 flex flex-col gap-4">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-0.5">{periodLabel} · {item.daysElapsed}/{item.totalDays} días</p>
        </div>
        <span className={`text-xs font-bold px-2 py-1 rounded-full ${isCash ? 'bg-blue-500/10 text-blue-400' : 'bg-emerald-500/10 text-emerald-400'}`}>
      ...
      {!hasMeta && (
        <p className="text-slate-600 text-xs text-center py-1">Sin metas configuradas</p>
      )}
      ...
      <div className="flex gap-2 pt-1">
        <button
          onClick={() => onMov(item)}
          className="flex-1 flex items-center justify-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-slate-300 hover:text-white text-xs font-bold px-3 py-2 rounded-lg transition-all"
        >
          <Plus size={14} />
          Movimiento
        </button>
      ...
      {item.minMonthlySpend === 0 && item.currentSpend > 0 && (
        <div className="flex items-center gap-2 bg-slate-900 rounded-xl p-3">
          ...
          <span className="text-slate-400 text-xs">Gasto del periodo: <span className="text-white font-bold">{MXN(item.currentSpend)}</span></span>
        </div>
      )}
      ...
      <button
        onClick={onToggle}
        className="w-full flex items-center justify-center gap-1.5 text-xs text-slate-500 hover:text-slate-300 pt-1 transition-colors"
      >
        <ChevronDown
          size={14}
          ...
          <div className="max-h-60 overflow-y-auto">
            <table className="w-full text-xs">
              <thead className="bg-slate-900 sticky top-0">
                <tr>
                  <th className="px-3 py-2 text-left text-slate-400 font-semibold">Fecha</th>
                  <th className="px-3 py-2 text-left text-slate-400 font-semibold">Movimientos</th>
                  <th className="px-3 py-2 text-right text-slate-400 font-semibold">Saldo Cierre</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <td className="px-3 py-2 text-slate-400">
                      {entry.transactions.length === 0 ? (
                        ...
                        entry.transactions.map((t) => (
                          <div key={t.id} className={t.type === 'DEPOSIT' ? 'text-emerald-400' : 'text-red-400'}>
                            {t.type === 'DEPOSIT' ? '+' : '-'}{MXN(t.amount)}
                            <span className="text-slate-500 ml-1 text-[10px] truncate block">{t.description}</span>
                          </div>
                        ))
                      )}
                    </td>
