...
const inputCls = 'w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-4 py-2 focus:outline-none focus:border-emerald-500 transition-colors placeholder-slate-600';
...
export default function LoanDetail() {
  ...
  const progressColor = progress >= 75 ? 'bg-emerald-500' : progress >= 40 ? 'bg-amber-500' : 'bg-sky-500';
  ...
  return (
    <div className="p-6 md:p-8 text-white font-sans max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
          ...
          {totalDue > 0 && (
            <div className="text-right shrink-0">
              <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-0.5">Deuda total pactada</p>
              <p className="text-xl font-extrabold text-emerald-400">{fmt(totalDue)}</p>
            </div>
          )}
        </div>
          ...
          <>
            <div className="grid grid-cols-3 gap-3 text-center mb-4">
              ...
              <div className="bg-slate-900/60 rounded-xl p-3">
                ...
                <p className={`text-base font-bold ${isPaid ? 'text-slate-500' : 'text-red-400'}`}>{fmt(debtActual)}</p>
              </div>
              <div className="bg-slate-900/60 rounded-xl p-3">
      ...
      {/* Estados especiales */}
      {isRequested && (
        <div className="bg-slate-800 border border-amber-500/30 rounded-2xl p-6 mb-6 flex flex-col items-center gap-4 text-center">
          <AlertTriangle size={32} className="text-amber-400" />
          <p className="text-amber-300 font-bold">Esperando aprobación del administrador</p>
          {(level.startsWith('NOVATO') || !familyCode) && (
            <a
              ...
              className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500 hover:text-white px-4 py-2 rounded-xl text-sm font-bold transition-all"
            >
      ...
      {isEsperando && (
        <div className="bg-slate-800 border border-amber-400/40 rounded-2xl p-6 mb-6">
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 mb-4">
            <p className="text-amber-300 font-bold text-sm mb-1">⚠️ Tu préstamo fue aprobado — Revisa las condiciones</p>
            <p className="text-amber-400/80 text-xs">Las condiciones pudieron haber sido modificadas. Revisa el monto, plazo y tasa antes de aceptar.</p>
          </div>
      ...
      {isPending && (
        <div className="bg-slate-800 border border-violet-500/30 rounded-2xl p-6 mb-6 flex flex-col items-center gap-3 text-center">
      ...
      {isRejected && (
        <div className="bg-slate-800 border border-red-500/30 rounded-2xl p-6 mb-6 flex flex-col items-center gap-3 text-center">
          <div className="w-14 h-14 rounded-full bg-red-500/10 flex items-center justify-center">
            <Ban size={28} className="text-red-400" />
          </div>
          <p className="text-red-400 font-semibold">Solicitud rechazada</p>
          <p className="text-slate-500 text-xs max-w-xs">Esta solicitud no fue aprobada. Puedes solicitar un nuevo préstamo cuando lo desees.</p>
        </div>
      )}
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-700/50">
                {loan.installments.map(inst => {
                  ...
                  return (
                    <tr key={inst.id} className={`hover:bg-slate-700/20 ${isPaidInst ? 'opacity-70' : ''}`}>
                      <td className="px-4 py-3 text-slate-400">{inst.number}</td>
                      <td className="px-4 py-3 text-slate-300 whitespace-nowrap">{fmtDate(inst.dueDate)}</td>
                      <td className="px-4 py-3 text-right text-slate-300 whitespace-nowrap">{fmt(inst.amountDue)}</td>
                      <td className={`px-4 py-3 text-right font-medium whitespace-nowrap ${isPaidInst ? 'text-emerald-400' : isPartial ? 'text-amber-400' : 'text-white'}`}>
                        {fmt(inst.amountPaid)}
                      </td>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6">
          ...
          <div className="space-y-2">
            {abonosHistory.map(abono => (
              <div key={abono.id} className="flex justify-between items-center text-sm bg-slate-900/40 rounded-lg px-4 py-2.5">
                ...
                <span className="text-emerald-400 font-bold">{fmt(abono.amount)}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {payRegistered ? (
              ...
              <>
                <div className="p-6 space-y-5 overflow-y-auto flex-1">
                  <div>
                    <label className="block text-sm text-slate-300 font-medium mb-1">¿A qué banco realizaste el depósito?</label>
                    ...
                    return (
                      <div className="bg-slate-900/70 border border-slate-600 rounded-xl p-5">
                        ...
                        <div className="space-y-3 text-sm">
                          <div className="flex justify-between"><span className="text-slate-400">Banco</span><span className="text-white font-semibold">{selected.bankName}</span></div>
                          <div className="flex justify-between"><span className="text-slate-400">Titular</span><span className="text-white font-semibold">{selected.accountHolder}</span></div>
                          <div className="flex justify-between items-center border-t border-slate-700 pt-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button onClick={() => copyText(selected.clabe)} className="text-slate-500 hover:text-emerald-400"><Copy size={14} /></button>
                            </div>
                          </div>
                          <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3 mt-1">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <button onClick={() => copyText(satCode)} className="text-slate-500 hover:text-amber-400"><Copy size={14} /></button>
                            </div>
                          </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="block text-sm text-slate-300 font-medium mb-1">
                        Referencia del SPEI <span className="text-slate-500 font-normal">(opcional)</span>
                      </label>
                    ...
                    <div>
                      <label className="block text-sm text-slate-300 font-medium mb-1">
                        ...
                        <span className="text-slate-500 font-normal ml-1">Captura, max 5 MB</span>
                      </label>
