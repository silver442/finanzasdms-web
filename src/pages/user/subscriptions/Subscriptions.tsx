...
function SubForm({ values, onChange, onSubmit, onCancel, saving, title, icon, cards }: {
  ...
  return (
    <div className="bg-surface-card rounded-2xl border border-surface-border shadow-2xl w-[95%] md:max-w-lg max-h-[85vh] flex flex-col">
      <div className="flex justify-between items-center p-4 md:p-6 border-b border-surface-border shrink-0">
        <h2 className="text-xl font-bold text-white flex items-center gap-2">
          {icon}
          {title}
        </h2>
        <button onClick={onCancel} className="text-text-secondary hover:text-white"><X size={20} /></button>
      </div>
    ...
    <div className="p-8 text-white font-sans max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row items-start md:items-center gap-4 mb-8">
        <div className="flex-1">
          ...
          <p className="text-text-secondary mt-2">Gastos recurrentes vinculados a tus tarjetas de crédito</p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          <button
            onClick={() => navigate('/subscriptions/shared')}
            className="flex-1 md:flex-none bg-surface-elevated hover:bg-surface-elevated text-text-200 px-5 py-2.5 rounded-xl font-bold transition-all flex items-center justify-center gap-2"
          >
            <Users size={18} />
            Compartidas
          </button>
          <button
            ...
            className="flex-1 md:flex-none bg-brand-green hover:bg-brand-green-light text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-lg shadow-brand-green/20 flex items-center justify-center gap-2"
          >
            <Plus size={20} />
            Nueva
          </button>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 text-text-muted bg-surface-card/50 rounded-2xl border border-surface-border">
          ...
          <p className="text-lg font-semibold text-text-secondary">Sin suscripciones registradas</p>
                ...
                {subs.map((sub) => (
                  <tr key={sub.id} className="hover:bg-surface-elevated/30 transition-colors">
                    ...
                    <td className="px-5 py-4 text-center">
                      <span className="bg-brand-green/10 text-brand-green-light text-xs font-bold px-3 py-1 rounded-full">
                        {FREQ_LABEL[sub.frequency] ?? sub.frequency}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-center text-text-primary font-medium">Día {sub.chargeDay}</td>
                    ...
                    <td className="px-5 py-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-text-muted hover:text-violet-400 hover:bg-violet-500/10 transition-all"
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <SubForm
            ...
            icon={<CalendarClock className="text-brand-green" size={20} />}
            cards={cards}
          />
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-surface-card rounded-2xl border border-surface-border shadow-2xl w-[95%] md:max-w-lg max-h-[85vh] flex flex-col">
            <div className="flex justify-between items-center p-4 md:p-6 border-b border-surface-border shrink-0">
              ...
              <button onClick={() => setShareTarget(null)} className="text-text-secondary hover:text-white"><X size={20} /></button>
            </div>
            <div className="overflow-y-auto flex-1 p-4 md:p-6 space-y-5">
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="bg-surface-elevated hover:bg-surface-elevated text-white px-4 py-3 rounded-lg font-medium transition-all shrink-0"
                  >
                    <Plus size={18} />
                  </button>
                </div>
              ...
              {shareEmails.length > 0 && (
                <div>
                  <p className="text-xs text-text-muted uppercase tracking-wider font-semibold mb-2">Por invitar</p>
                  <div className="space-y-2">
                    {shareEmails.map((email) => (
                      <div key={email} className="flex items-center justify-between bg-surface-base/50 rounded-xl px-4 py-2.5">
                        <span className="text-sm text-text-primary">{email}</span>
                        <button onClick={() => setShareEmails((prev) => prev.filter((e) => e !== email))} className="text-text-muted hover:text-red-400 transition-all">
                          <X size={14} />
                        </button>
                      </div>
                    ))}
              ...
              {shareTarget.sharedSubscription && shareTarget.sharedSubscription.members.length > 0 && (
                <div>
                  <p className="text-xs text-text-muted uppercase tracking-wider font-semibold mb-2">Miembros actuales</p>
                  <div className="space-y-2">
                    {shareTarget.sharedSubscription.members.map((m) => (
                      <div key={m.id} className="flex items-center justify-between bg-surface-base/50 rounded-xl px-4 py-2.5">
                        <span className="text-sm text-text-primary">{m.user?.name ?? m.email}</span>
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                          m.status === 'ACCEPTED' ? 'bg-brand-green/20 text-brand-green-light' :
                  ...
                  {shareTarget.sharedSubscription.members.filter((m) => m.status === 'ACCEPTED').length > 0 && (
                    <div className="mt-3 bg-brand-green/10 border border-brand-green/20 rounded-xl px-4 py-3">
                      <p className="text-sm text-brand-green-light font-semibold">
            ...
            <div className="flex gap-3 p-4 md:p-6 border-t border-surface-border shrink-0">
              <button
                type="button"
