...
function RendimientoRow({ open, close, label }: { open: number; close: number; label: string }) {
  ...
  return (
    <tr className="hover:bg-surface-elevated/30">
      <td className="p-3 text-text-primary font-medium">{label}</td>
      <td className="p-3 text-right text-text-secondary">{fmt(open)}</td>
      <td className="p-3 text-right text-white font-medium">{fmt(close)}</td>
      <td className={`p-3 text-right font-medium ${isPos ? 'text-brand-green-light' : 'text-red-400'}`}>
        {isPos ? '+' : ''}{fmt(gain)}
      </td>
      <td className={`p-3 text-right font-bold ${isPos ? 'text-brand-green-light' : 'text-red-400'}`}>
        {isPos ? '+' : ''}{fmtPct(rend)}
      </td>
    </tr>
  );
...
export default function PortfolioDetail() {
  ...
  if (loading) {
    return (
      <div className="p-8 text-white font-sans max-w-7xl mx-auto">
        <div className="text-center text-text-secondary py-20">Cargando cuenta...</div>
      </div>
    );
  }
  ...
  if (!account) {
    return (
      <div className="p-8 text-white font-sans max-w-7xl mx-auto">
        <div className="text-center text-text-secondary py-20">Cuenta no encontrada.</div>
        <div className="text-center mt-4">
          <Link to="/portfolio" className="text-brand-green-light hover:underline">← Volver al Portafolio</Link>
        </div>
      </div>
    );
  }
  ...
  const rendTableHead = (
    <thead className="text-text-secondary bg-surface-base/50 text-xs uppercase">
      <tr>
  ...
  return (
    <div className="p-8 text-white font-sans max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        <Link to="/portfolio"
          className="inline-flex items-center gap-2 text-text-secondary hover:text-white transition-colors mb-4 text-sm font-medium">
          <ArrowLeft size={16} />
          Portafolio
        </Link>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-text-secondary text-sm">{account.category}</p>
          </div>

          <div className="flex gap-6">
            <div className="text-right">
              <p className="text-xs text-text-secondary font-medium uppercase tracking-wider">Depositado</p>
              <p className="text-xl font-bold text-text-200">{fmt(deposited)}</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-text-secondary font-medium uppercase tracking-wider">Valor Real</p>
              <p className="text-xl font-bold text-white">{fmt(realValue)}</p>
            </div>
            {!isCash && (
              <div className="text-right">
                <p className="text-xs text-text-secondary font-medium uppercase tracking-wider">Ganancia</p>
                <p className={`text-xl font-extrabold ${gain >= 0 ? 'text-brand-green-light' : 'text-red-400'}`}>
                  {gain >= 0 ? '+' : ''}{fmt(gain)}
                </p>
              </div>
            )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-surface-card rounded-2xl border border-surface-border p-6 h-fit">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {['Depósito', 'Retiro', 'Gasto'].map((type) => (
                <button key={type} type="button"
                  ...
                  className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-all ${
                    form.type === type
                      ? type === 'Depósito' ? 'bg-brand-green text-white shadow' : 'bg-red-500 text-white shadow'
            ...
            <div>
              ...
              <input type="date" required value={form.date}
                ...
                className="w-full bg-surface-base border border-surface-border text-white rounded-lg px-4 py-2 focus:outline-none focus:border-brand-green cursor-pointer" />
            </div>

            <div>
              <label className="block text-text-secondary text-sm font-medium mb-1">Importe (MXN)</label>
            ...
            <button type="submit" disabled={saving}
              className="w-full bg-brand-green hover:bg-brand-green-light disabled:opacity-50 text-white py-2.5 rounded-lg font-bold transition-colors mt-2">
        ...
        <div className="lg:col-span-2 bg-surface-card rounded-2xl border border-surface-border overflow-hidden">
          <div className="p-5 border-b border-surface-border">
            <h2 className="text-lg font-bold text-white">Historial de Movimientos</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="text-text-secondary bg-surface-base/50">
                <tr>
              ...
              <tbody className="divide-y divide-slate-700/50">
                {transactions.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="p-8 text-center text-text-muted">Sin movimientos registrados</td>
                  </tr>
                ) : (
                  transactions.map((tx) => {
                    ...
                    return (
                      <tr key={tx.id} className="hover:bg-surface-elevated/30">
                        <td className="p-4 text-text-primary">{fmtDate(tx.date)}</td>
                        <td className={`p-4 font-medium ${isIncome ? 'text-brand-green-light' : 'text-red-400'}`}>
                          {TX_TYPE_LABEL[tx.type] ?? tx.type}
                        </td>
                        <td className="p-4 text-text-secondary text-xs">{tx.description}</td>
                        <td className={`p-4 text-right font-bold ${amount >= 0 ? 'text-brand-green-light' : 'text-red-400'}`}>
                          {amount >= 0 ? '+' : ''}{fmt(amount)}
                        </td>
                      </tr>
                    );
      ...
      {snapshots.length === 0 ? (
        <div className="bg-surface-card rounded-2xl border border-surface-border p-8 text-center">
          <p className="text-text-secondary text-sm">Los cortes mensuales se registran automáticamente el día 1 de cada mes.</p>
          <p className="text-text-muted text-xs mt-1">Las tablas de rendimiento aparecerán aquí una vez se genere el primer snapshot.</p>
        </div>
      ) : (
          ...
          <div className="bg-surface-card rounded-2xl border border-surface-border overflow-hidden">
            <div className="p-5 border-b border-surface-border">
              <h2 className="text-lg font-bold text-white">
                Rendimiento Mensual <span className="text-text-secondary font-normal text-sm">({new Date().getFullYear()})</span>
              </h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
