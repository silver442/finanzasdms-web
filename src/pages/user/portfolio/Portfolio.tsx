...
export default function Portfolio() {
  ...
  const renderRend = (value: number | null, isCash: boolean) => {
    if (isCash) return <span className="text-text-muted text-xs">—</span>;
    if (value === null) return <span className="text-text-muted text-xs">—</span>;
    const isPos = value >= 0;
    return (
      <span className={`font-bold text-xs ${isPos ? 'text-brand-green-light' : 'text-red-400'}`}>
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-surface-card rounded-2xl border border-surface-border shadow-2xl w-full max-w-md p-6">
          ...
          <form onSubmit={onSubmit} className="space-y-4">
              ...
              <div className="flex bg-surface-base rounded-lg p-1 border border-surface-border">
                {(['new', 'existing'] as const).map((s) => (
                  <button key={s} type="button" onClick={() => setAccountStatus(s)}
                    className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${accountStatus === s ? 'bg-brand-green text-white shadow' : 'text-text-secondary hover:text-white'}`}>
            ...
            <div>
              <label className="block text-text-secondary text-sm font-medium mb-1">Tipo de Inversión</label>
              <select value={form.investmentType} onChange={(e) => onTypeChange(e.target.value)}
                className="w-full bg-surface-base border border-surface-border text-white rounded-lg px-4 py-2 focus:outline-none focus:border-brand-green">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-text-secondary text-sm font-medium mb-1">Categoría</label>
                ...
                {isCustomCat && (
                  <input type="text" placeholder="Escribe la categoría..." required
                    ...
                    className="w-full mt-2 bg-surface-base border border-brand-green/50 text-white rounded-lg px-4 py-2 focus:outline-none focus:border-brand-green" />
                )}
              </div>
              <div>
                <label className="block text-text-secondary text-sm font-medium mb-1">Nombre</label>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                <label className="block text-text-secondary text-sm font-medium mb-1">Color del badge</label>
                <div className="flex items-center gap-3">
                  <input type="color" value={form.color}
                    onChange={(e) => isEdit ? setEditForm((p) => ({ ...p, color: e.target.value })) : setNewAccount((p) => ({ ...p, color: e.target.value }))}
                    className="h-9 w-14 rounded cursor-pointer bg-surface-base border border-surface-border p-0.5" />
            ...
            {!isEdit && accountStatus === 'existing' && (
              <div className={`pt-2 border-t border-surface-border mt-2 ${isCashForm ? '' : 'grid grid-cols-2 gap-4'}`}>
                <div>
                  <label className="block text-text-secondary text-sm font-medium mb-1">Depositado Inicial</label>
                ...
                {!isCashForm && (
                  <div>
                    <label className="block text-text-secondary text-sm font-medium mb-1">Valor Real Actual</label>
                ...
                {isCashForm && (
                  <p className="text-xs text-text-muted mt-1">El Valor Real siempre es igual al Depositado en cuentas corrientes.</p>
                )}
              </div>
            )}
            ...
            <div className="flex gap-4 mt-6">
              <button type="button" onClick={onClose}
                className="flex-1 bg-surface-elevated hover:bg-surface-elevated text-white py-2 rounded-lg transition-colors font-medium">Cancelar</button>
              <button type="submit"
                className="flex-1 bg-brand-green hover:bg-brand-green-light text-white py-2 rounded-lg transition-colors font-bold">
  ...
  return (
    <div className="p-8 text-white font-sans max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-brand-green-light">Portafolio de Inversiones</h1>
          <p className="text-text-secondary mt-2">Resumen global y valor real</p>
        </div>
        <button onClick={() => setIsNewAccountModalOpen(true)}
          className="bg-brand-green hover:bg-brand-green-light text-white px-4 py-2 rounded-lg font-bold transition-colors shadow-lg w-full md:w-auto">
          + Nueva Cuenta
        </button>
      </div>
      ...
      <div className="bg-surface-card rounded-2xl border border-surface-border shadow-xl overflow-hidden">
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="p-12 text-center text-text-secondary">Cargando portafolio...</div>
          ) : (
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead>
                <tr className="bg-surface-base/80 text-text-primary uppercase tracking-wider border-b border-surface-border">
                  ...
                  items.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-surface-elevated/30 transition-colors">
                        ...
                        <td className="p-4 text-text-primary">{item.category}</td>
                        <td className="p-4">
                          <Link to={`/portfolio/${item.id}`}
                            className="font-bold text-white hover:text-brand-green-light transition-colors">
                            {item.name}
                          </Link>
                        </td>
                        <td className="p-4 text-right text-text-primary">{formatCurrency(deposited)}</td>
                        <td className="p-4 text-right font-bold text-white">{formatCurrency(realValue)}</td>
                        <td className={`p-4 text-right font-medium ${isCash ? 'text-text-muted' : isPositive ? 'text-brand-green-light' : 'text-red-400'}`}>
                        ...
                        <td className="p-4">
                          <div className="flex items-center justify-center gap-2">
                            <button onClick={() => openEdit(item)} title="Editar"
                              className="text-text-secondary hover:text-brand-green-light transition-colors p-1 rounded hover:bg-brand-green/10">
                              <Pencil size={15} />
                            </button>
                            <button onClick={() => handleDelete(item)} title="Eliminar"
                              className="text-text-secondary hover:text-red-400 transition-colors p-1 rounded hover:bg-red-500/10">
                              <Trash2 size={15} />
                            </button>
                          </div>
                        </td>
              ...
              {items.length > 0 && (
                <tfoot>
                  <tr className="bg-surface-base/90 border-t-2 border-surface-border">
                    <td colSpan={3} className="p-4 text-right font-bold text-text-primary uppercase tracking-wider">Totales:</td>
                    <td className="p-4 text-right font-bold text-text-primary">{formatCurrency(totalDeposited)}</td>
                    <td className="p-4 text-right font-extrabold text-white text-lg">{formatCurrency(totalRealValue)}</td>
                    <td className={`p-4 text-right font-extrabold text-lg ${totalGain >= 0 ? 'text-brand-green' : 'text-red-500'}`}>
