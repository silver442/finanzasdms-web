          ...
          {loans.map(loan => {
            ...
            return (
              <button
                ...
                className="bg-slate-800 border border-slate-700 rounded-2xl shadow-xl overflow-hidden flex flex-col text-left w-full hover:bg-slate-700/60 hover:border-sky-500/40 transition-all group"
              >
                {/* Header */}
                ...
                {/* Cliente */}
                <div className="px-5 py-3 flex items-center gap-2 border-b border-slate-700/50">
                ...
                <div className="p-5 flex-1 space-y-3">
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-0.5">Pendiente</p>
                      <p className="text-lg font-extrabold text-amber-400">{fmt(pending)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-0.5">Pagado</p>
                      <p className="text-lg font-extrabold text-emerald-400">{fmt(paid)}</p>
                    </div>
                  </div>
                  ...
                  {nextInst && (
                    <div className="bg-slate-900/50 rounded-lg px-3 py-2 text-xs">
                      <span className="text-slate-500">Próximo venc.: </span>
                      <span className="text-white font-semibold">{fmtDate(nextInst.dueDate)}</span>
                      <span className="text-slate-400"> · </span>
                      <span className="text-amber-400 font-semibold">{fmt(Number(nextInst.amountDue) - Number(nextInst.amountPaid))}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="px-5 py-3 border-t border-slate-700 flex items-center justify-between text-xs text-slate-500">
                  ...
                  <ChevronRight size={16} className="text-slate-500 group-hover:text-sky-400 transition-colors" />
                </div>
              </button>
            );
          })}
