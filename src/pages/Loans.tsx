...
const STATUS_CONFIG: Record<Loan['status'], { label: string; border: string; badge: string }> = {
  ...
  PAID:                   { label: 'Liquidado',                border: 'border-emerald-500/30', badge: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' },
  REQUESTED:              { label: 'En revisión',              border: 'border-amber-500/30',  badge: 'bg-amber-500/20 text-amber-400 border-amber-500/30' },
  ESPERANDO_CONFIRMACION: { label: '⚠️ Confirmar condiciones', border: 'border-amber-400/40',  badge: 'bg-amber-500/20 text-amber-300 border-amber-500/30' },
  ...
  CANCELADO:              { label: 'Cancelado',                border: 'border-slate-600',     badge: 'bg-slate-700/50 text-slate-400 border-slate-600' },
...
const STATUS_ICON: Record<Loan['status'], React.ReactElement> = {
  ...
  PAID:                   <CheckCircle2 size={15} className="text-emerald-400" />,
  REQUESTED:              <AlertTriangle size={15} className="text-amber-400" />,
  ESPERANDO_CONFIRMACION: <AlertTriangle size={15} className="text-amber-300" />,
  PENDIENTE_TRANSFERIR:   <Banknote size={15} className="text-violet-400" />,
  REJECTED:               <XCircle size={15} className="text-red-400" />,
  CANCELADO:              <XCircle size={15} className="text-slate-400" />,
      ...
      {!slotsAvailable && (
        <div className="mb-4 flex items-center gap-3 bg-amber-500/10 border border-amber-500/30 rounded-xl px-5 py-4">
          ...
          <div>
            ...
            <p className="text-amber-400/70 text-xs mt-0.5">Se han alcanzado los 20 créditos activos simultáneos.</p>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
          {visibleLoans.map(loan => {
            ...
            const progressColor =
              ...
              progress >= 40 ? 'bg-amber-500' : 'bg-sky-500';

            return (
              <button
                key={loan.id}
                onClick={() => navigate(`/loans/${loan.id}`)}
                className={`bg-slate-800 border ${cfg.border} rounded-2xl shadow-xl overflow-hidden flex flex-col text-left w-full hover:bg-slate-700/60 transition-all group`}
              >
                {/* Card header */}
                <div className="p-5 border-b border-slate-700 flex justify-between items-start gap-2">
                ...
                <div className="p-5 flex-1 space-y-3">
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-0.5">Deuda actual</p>
                      <p className={`text-lg font-extrabold ${loan.status === 'PAID' ? 'text-slate-500' : 'text-red-400'}`}>
                        {formatCurrency(debtActual)}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-0.5">Pagado</p>
                      <p className="text-lg font-extrabold text-emerald-400">{formatCurrency(totalPaid)}</p>
                    </div>
                  </div>
                  ...
                  {nextInstallment && loan.status === 'ACTIVE' && (
                    <div className="bg-slate-900/50 rounded-lg px-3 py-2 text-xs">
                      ...
                      <span className="text-amber-400 font-semibold">{formatCurrency(Number(nextInstallment.amountDue) - Number(nextInstallment.amountPaid))}</span>
                    </div>
                  )}
                </div>
